...
async function tests() {
    ...
    const test1 = await client.ai.text.translate.fulfill({ text: "How's it going?", to: 'es' })
    prettyPrint(test1)

    // Bulk mode
    const bulk = await client.ai.text.translate.fulfill({
    ...
    // category `generalnn`
    const test3 = await client.ai.text.translate.fulfill({
        text: 'A sample text',
        to: 'es',
    ...
    const test4 = await client.ai.text.translate.fulfill({
