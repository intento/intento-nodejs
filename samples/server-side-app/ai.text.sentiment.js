...
async function tests() {
    const test1 = await client.ai.text.sentiment.fulfill({
    ...
    const data = await client.ai.text.sentiment.providers({ bulk: true })

    data.forEach(p => console.info(p.name))

    // Bulk mode
    const bulk = await client.ai.text.sentiment.fulfill({
