...
async function tests() {
    // basic translation
    const basicTranslation = await client.ai.text.translate.fulfill({
        text: "How's it going?",
        to: 'es',
    })
    ...
    // Sentiment analysis
    const sentimentAnalysis = await client.ai.text.sentiment.fulfill({
    ...
    // Text meanings
    const textMeanings = await client.ai.text.dictionary.fulfill({
        text: 'meaning',
        from: 'en',
        to: 'ru',
    })
    ...
    const translationProviders = await client.ai.text.translate.providers()
    translationProviders.forEach(p => console.info(p.name))

    // Basic smart routing
    const basicSmartRouting = await client.ai.text.translate.fulfill({
        text: "How's it going?",
        to: 'es',
    })
    ...
    const withKeys = await client.ai.text.translate.fulfill({
        text: 'A sample text',
        to: 'es',
